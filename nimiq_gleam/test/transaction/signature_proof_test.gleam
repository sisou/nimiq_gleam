...
import nimiq/key/public_key.{EcDsaPublicKey, EdDsaPublicKey}
import nimiq/key/signature.{EcDsaSignature, EdDsaSignature}
...
pub fn regualar_webauthn_signature_proof_test() {
  ...
  assert signature_proof.serialize_to_bits(proof) == expected

  let assert Ok(proof) = signature_proof.deserialize_all(expected)
  assert proof.public_key == public
  assert proof.signature == signature
  assert proof.webauthn_fields == Some(webauthn_fields)
}
