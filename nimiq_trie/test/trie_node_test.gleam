...
pub fn child_index_test() {
  ...
  should.equal(branch_node |> node.child_index(child_key_1), Ok(15))
  should.equal(branch_node |> node.child_index(child_key_2), Ok(10))
  should.equal(branch_node |> node.child_index(child_key_3), Ok(0))
  should.equal(branch_node |> node.child_index(child_key_4), Ok(13))

  should.equal(
    branch_node |> node.child_index(child_key_1 |> key_nibbles.slice(0, 7)),
    Ok(15),
  )

  should.equal(leaf_node |> node.child_index(child_key_2), Ok(10))

  let assert Ok(child_key_5) = key_nibbles.from_str("c0b986d50")
  should.equal(
    branch_node |> node.child_index(child_key_5),
...
pub fn child_test() {
  ...
  should.equal(
    branch_node |> node.child(child_key_1) |> result.map(fn(c) { c.hash }),
    Ok(<<"child_1">> |> blake2b.hash()),
  )
  should.equal(
    branch_node |> node.child(child_key_2) |> result.map(fn(c) { c.hash }),
    Ok(<<"child_2">> |> blake2b.hash()),
  )
  should.equal(
    branch_node |> node.child(child_key_3) |> result.map(fn(c) { c.hash }),
    Ok(<<"child_3">> |> blake2b.hash()),
  )
  should.equal(
    branch_node |> node.child(child_key_4) |> result.map(fn(c) { c.hash }),
  ...
  should.equal(
    leaf_node |> node.child(child_key_2) |> result.map(fn(c) { c.hash }),
    Error(error.ChildDoesNotExist),
  )

  let assert Ok(child_key_5) = key_nibbles.from_str("c0b986d50")
  should.equal(
    branch_node |> node.child(child_key_5) |> result.map(fn(c) { c.hash }),
    Error(error.WrongPrefix),
  )
}
...
pub fn child_key_test() {
  ...
  should.equal(
    leaf_node |> node.child_key(child_key_2),
    Error(error.ChildDoesNotExist),
  )

  let assert Ok(child_key_5) = key_nibbles.from_str("c0b986d50")
  should.equal(
    branch_node |> node.child_key(child_key_5),
    Error(error.WrongPrefix),
  )
}
...
pub fn put_remove_child_test() {
  ...
  should.equal(node |> node.child(child_key_1), Error(error.ChildDoesNotExist))

  let assert Ok(node) = node |> node.remove_child(child_key_2)
  should.equal(node |> node.child(child_key_2), Error(error.ChildDoesNotExist))

  let assert Ok(node) = node |> node.remove_child(child_key_3)
  should.equal(node |> node.child(child_key_3), Error(error.ChildDoesNotExist))

  let assert Ok(node) = node |> node.remove_child(child_key_4)
  should.equal(node |> node.child(child_key_4), Error(error.ChildDoesNotExist))
...
pub fn put_value_test() {
  ...
  should.equal(
    root_node |> node.put_value(<<99>>),
    Error(error.RootCantHaveValue),
  )
  should.equal(root_node.value, None)
}
...
pub fn remove_value_test() {
  ...
  should.be_true(leaf_node |> node.is_empty())
